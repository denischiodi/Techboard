...
const CATEGORY_CONFIG: {
...
}[] = [
  {
    ...
    label: "Perfis",
    icon: Users,
    description: "Papéis dos recursos (Funcional, Técnico, etc.)",
  },
  {
    key: "fronts",
    label: "Frentes",
    icon: Layers,
    description: "Módulos/frentes de trabalho (FI, MM, SD, etc.)",
  },
  {
    key: "resourceStatuses",
    label: "Status de Recurso",
    icon: Activity,
    description: "Status possíveis para recursos",
  },
  {
    key: "projectStatuses",
    label: "Status de Projeto",
    icon: FolderKanban,
    description: "Status possíveis para projetos",
  },
  {
    key: "absenceTypes",
    label: "Tipos de Ausência",
    icon: CalendarOff,
    description: "Tipos de férias e ausências",
  },
  {
    key: "allocationTypes",
    label: "Tipos de Alocação",
    icon: Briefcase,
    description: "Tipos de alocação (Projeto, Interna, etc.)",
  },
  {
    key: "allocationStatuses",
    label: "Status de Alocação",
    icon: CheckCircle,
    description: "Status possíveis para alocações",
  },
  {
    key: "contractTypes",
    label: "Tipos de Contratação",
    icon: Briefcase,
    description: "Tipos de contrato (CLT, PJ, etc.)",
  },
  {
    key: "dashboardCheckStatuses",
    label: "Status p/ Verificar Recurso",
    icon: FolderKanban,
    description:
      "Status de projeto que devem ser verificados no Dashboard (Projetos Faltando Recurso)",
  },
];

export default function Settings() {
  const { data: lookups, refetch } = trpc.settings.getLookups.useQuery();
  const addItem = trpc.settings.addLookup.useMutation({
    onSuccess: () => refetch(),
  });
  const updateItem = trpc.settings.updateLookup.useMutation({
    onSuccess: () => refetch(),
  });
  const deleteItem = trpc.settings.deleteLookup.useMutation({
    onSuccess: () => refetch(),
  });
  ...
  const handleAdd = () => {
    ...
    addItem.mutate(
      { category: activeTab, value: newValue.trim() },
      {
        onSuccess: () => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Gerencie os valores auxiliares utilizados em todo o sistema
        </p>
      </div>

      <Tabs
        value={activeTab}
        onValueChange={v => setActiveTab(v as LookupCategory)}
        className="space-y-4"
      >
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  Total: {currentItems.length} itens (
                  {currentItems.filter(i => i.active).length} ativos)
                </p>
              </CardContent>
            </Card>
          </TabsContent>
