...
export default function StandardConfigurations() {
  ...
  const openBdcq = (template?: any) => { setBdcqForm(template ? { id: template.id, question: template.question, category: template.category || "", modules: template.modules || [], scopeItemKeys: template.scopeItemKeys || [], required: Boolean(template.required), active: template.active ?? 1 } : emptyBdcq()); setBdcqScopeSearch(""); setBdcqOpen(true); };
  const saveBdcq = () => {
  ...
  return <div className="space-y-6">
    <div><h1 className="text-2xl font-bold">Configurações do TechMove</h1><p className="text-sm text-muted-foreground">Processos, equipes e modelos globais que orientam a trilha completa dos projetos.</p></div>
    <Tabs defaultValue="process-models" className="space-y-3">
      <TabsList className="grid h-auto w-full grid-cols-2 gap-1 p-1 md:grid-cols-4">
        <TabsTrigger className="h-11 min-w-0 whitespace-normal px-2 text-center leading-tight" value="process-models">Processos e equipes</TabsTrigger>
      ...
      <TabsContent value="process-models"><ProcessModelsConfig /></TabsContent>
    ...
    <Dialog open={bdcqOpen} onOpenChange={setBdcqOpen}><DialogContent className="max-h-[92vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>{bdcqForm.id ? "Editar pergunta padrão" : "Nova pergunta padrão"}</DialogTitle></DialogHeader><div className="grid gap-4"><div><Label>Pergunta *</Label><Textarea rows={4} value={bdcqForm.question} onChange={event => setBdcqForm(form => ({ ...form, question: event.target.value }))} /></div><div><Label>Categoria</Label><Input value={bdcqForm.category} onChange={event => setBdcqForm(form => ({ ...form, category: event.target.value }))} /></div><div className="flex flex-wrap gap-5"><label className="flex items-center gap-2"><Switch checked={bdcqForm.required} onCheckedChange={required => setBdcqForm(form => ({ ...form, required }))} /><span className="text-sm font-medium">Obrigatória</span></label><label className="flex items-center gap-2"><Switch checked={bdcqForm.active === 1} onCheckedChange={active => setBdcqForm(form => ({ ...form, active: active ? 1 : 0 }))} /><span className="text-sm font-medium">Ativa</span></label></div><div><Label>Módulos relacionados</Label><div className="mt-2 flex flex-wrap gap-2">{moduleOptions.map(module => <Button key={module} type="button" size="sm" variant={bdcqForm.modules.includes(module) ? "default" : "outline"} onClick={() => setBdcqForm(form => ({ ...form, modules: toggle(form.modules, module) }))}>{module}</Button>)}</div></div><div><div className="flex flex-wrap items-center justify-between gap-2"><Label>Scope items relacionados</Label><Badge variant="secondary">{bdcqForm.scopeItemKeys.length} selecionado(s)</Badge></div><div className="relative mt-2"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={bdcqScopeSearch} onChange={event => setBdcqScopeSearch(event.target.value)} placeholder="Buscar por código, nome ou módulo" /></div><div className="mt-2 flex flex-wrap items-center gap-2"><Button type="button" size="sm" variant="outline" disabled={!filteredBdcqScopeOptions.length} onClick={() => setBdcqForm(form => ({ ...form, scopeItemKeys: [...new Set([...form.scopeItemKeys, ...filteredBdcqScopeOptions.map(item => item.key)])] }))}>Selecionar resultados ({filteredBdcqScopeOptions.length})</Button><Button type="button" size="sm" variant="ghost" disabled={!bdcqForm.scopeItemKeys.length} onClick={() => setBdcqForm(form => ({ ...form, scopeItemKeys: [] }))}>Limpar seleção</Button></div><div className="mt-2 max-h-64 space-y-1 overflow-y-auto rounded-md border p-2">{filteredBdcqScopeOptions.map(item => <label key={`${item.module}:${item.key}`} className="flex cursor-pointer items-center gap-2 rounded p-1.5 text-sm hover:bg-muted"><Checkbox checked={bdcqForm.scopeItemKeys.includes(item.key)} onCheckedChange={() => setBdcqForm(form => ({ ...form, scopeItemKeys: toggle(form.scopeItemKeys, item.key) }))} /><span>{item.code ? `${item.code} - ` : ""}{item.name}</span><Badge variant="outline" className="ml-auto">{item.module}</Badge></label>)}{filteredBdcqScopeOptions.length === 0 && <p className="py-4 text-center text-sm text-muted-foreground">Nenhum scope item encontrado.</p>}</div><p className="mt-1 text-xs text-muted-foreground">A busca não remove os itens já selecionados. Sem módulos e scope items, a pergunta será geral. Com ambos, o projeto deve corresponder aos dois.</p></div></div><DialogFooter><Button variant="outline" onClick={() => setBdcqOpen(false)}>Cancelar</Button><Button onClick={saveBdcq} disabled={!bdcqForm.question.trim() || createBdcq.isPending || updateBdcq.isPending}>Salvar</Button></DialogFooter></DialogContent></Dialog>
