              ...
              {filtered.length === 0 ? (
                ...
                <TableRow key={c.id} className={c.status === "Concluído" ? "opacity-60" : ""}>
                  ...
                  <TableCell>
                    ...
                    <div className="mt-1 flex flex-wrap gap-1"><Badge variant="outline" className="text-[10px]">{c.source === "bdcq" ? "Origem: BDCQ" : ["template", "delivery_template"].includes(c.source) ? `Origem: Configurações do Tech · v${c.templateVersion || 1}` : c.source === "dcd" ? "Origem: DCD" : "Origem: manual"}</Badge>{(c.scopeItemIds || []).map((id: string) => { const scope = scopeItems.find((item: any) => item.id === id); return scope ? <Badge key={id} variant="secondary" className="text-[10px]">Scope: {scope.code || scope.name}</Badge> : null; })}</div>
                  </TableCell>
