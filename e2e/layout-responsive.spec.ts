...
async function expectInteractiveControlsInsideViewport(page: Page) {
  ...
  expect(
    offenders,
    `Controles fora do viewport: ${JSON.stringify(offenders)}`
  ).toEqual([]);
}

for (const [name, route] of routes) {
  test(`${name}: não corta a página nem controles no viewport configurado`, async ({
    page,
  }) => {
...
test("diálogo de recurso permanece acessível e rolável em tela pequena", async ({
  page,
  isMobile,
}) => {
...
test("navegação TechBoard por menu funciona em desktop e mobile", async ({
  page,
  isMobile,
}) => {
