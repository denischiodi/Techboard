...
function findHeaderRow(rows: CellRow[]) {
  ...
  inspectedRows.forEach((row, index) => {
    ...
    const score = (Object.keys(HEADER_ALIASES) as Field[]).filter(
      field => findHeaderIndex(row, field) >= 0
    ).length;
...
function normalizePriority(value: string) {
  ...
  if (priority.startsWith("medium") || priority.startsWith("med"))
    return "Média";
  return value || "Normal";
}
...
export function parseDdaWorkbook(
  ...
  const candidates = workbook.SheetNames.map((name, order) => ({
    name,
    order,
    score: ["scope", "capabilit", "dda"].reduce(
      (score, token) =>
        score + (normalizeDdaHeader(name).includes(token) ? 1 : 0),
      0
    ),
  ...
  const idFactory =
    options.idFactory ??
    (index =>
      `scope-${Date.now()}-${index}-${Math.random().toString(36).slice(2, 8)}`);

  for (const candidate of candidates) {
    const sheet = workbook.Sheets[candidate.name];
    if (!sheet) continue;
    const rows = XLSX.utils.sheet_to_json<CellRow>(sheet, {
      header: 1,
      defval: "",
      raw: false,
    });
    ...
    rows.slice(headerRowIndex + 1).forEach((row, index) => {
      ...
      const key = resolvedCode
        ? normalizeDdaHeader(resolvedCode)
        : `__missing_code_${index}`;
      const existing = itemsByCode.get(key);

      if (existing) {
        existing.module = joinDistinct(existing.module, module);
        existing.processArea = joinDistinct(
          existing.processArea,
          resolvedProcessArea
        );
        existing.lob = joinDistinct(existing.lob || "", lob);
        existing.description = joinDistinct(
      ...
      itemsByCode.set(key, {
        ...
        module,
        code: resolvedCode,
        name: name || code || `Scope item sem código ${index + 1}`,
