...
export const DELIVERY_TYPES = [
  ...
  "unit_test", "cycle_1", "cycle_2", "risk", "issue", "cutover",
  "go_live", "closure",
] as const;
...
export type DeliveryTemplateInput = {
  ...
  approvalPolicy?: { mode: "none" | "any" | "all" | "minimum"; minimumApprovals?: number };
...
const jsonColumns = new Set([
  "modules", "scopeItemKeys", "projectIds", "dependencyTemplateIds",
  "evidenceRequirements", "approvalPolicy", "payload", "scopeItemIds",
  "dependencyItemIds", "evidences",
]);
const serialize = (key: string, value: unknown) => jsonColumns.has(key) ? JSON.stringify(value ?? (key === "approvalPolicy" || key === "payload" ? {} : [])) : value;
...
async function insert(table: string, data: Record<string, unknown>) {
  ...
  const values = entries.map(([, value], index) => `$${index + 1}${typeof value === "object" ? "::jsonb" : ""}`).join(",");
  const result = await pool.query(`INSERT INTO "${table}" (${columns}) VALUES (${values}) RETURNING *`, entries.map(([key, value]) => serialize(key, value)));
  return result.rows[0];
}

export async function listTemplates(options: { type?: string; includeArchived?: boolean } = {}) {
  const pool = getPgPool();
  if (!pool) return [];
...
export async function createTemplate(input: DeliveryTemplateInput, userId: string) {
  ...
  const created = await insert("delivery_templates", {
    id, ...input, description: input.description || "", instructions: input.instructions || "",
    phase: input.phase || "Prepare", modules: input.modules || [], scopeItemKeys: input.scopeItemKeys || [],
    projectIds: input.projectIds || [], required: input.required ?? true, sortOrder: input.sortOrder || 0,
    dependencyTemplateIds: input.dependencyTemplateIds || [], ownerRole: input.ownerRole || "consultant",
    dueOffsetDays: input.dueOffsetDays || 0, evidenceRequirements: input.evidenceRequirements || [],
    approvalPolicy: input.approvalPolicy || { mode: "none", minimumApprovals: 1 },
    completionCriteria: input.completionCriteria || "", payload: input.payload || {}, version: 1,
    effectiveFrom: input.effectiveFrom || "", active: input.active ?? true, createdBy: userId,
  }) as Record<string, unknown>;
  await insert("delivery_template_versions", { id: `dtv_${nanoid(20)}`, templateId: id, version: 1, snapshot: created, changedBy: userId });
  return created;
}

export async function updateTemplate(id: string, patch: Partial<DeliveryTemplateInput>, userId: string) {
  const pool = getPgPool();
  if (!pool) return { id, ...patch };
  const current = await pool.query('SELECT * FROM "delivery_templates" WHERE "id"=$1 AND "archivedAt" IS NULL', [id]);
  if (!current.rows[0]) return null;
  const allowed = Object.entries(patch).filter(([key, value]) => value !== undefined && !["id", "createdBy"].includes(key));
  const nextVersion = Number(current.rows[0].version || 1) + 1;
  const sets = allowed.map(([key], index) => `"${key}"=$${index + 2}${jsonColumns.has(key) ? "::jsonb" : ""}`);
  sets.push(`"version"=$${allowed.length + 2}`, '"updatedAt"=now()');
  const values = allowed.map(([key, value]) => serialize(key, value));
  const result = await pool.query(`UPDATE "delivery_templates" SET ${sets.join(",")} WHERE "id"=$1 RETURNING *`, [id, ...values, nextVersion]);
  await insert("delivery_template_versions", { id: `dtv_${nanoid(20)}`, templateId: id, version: nextVersion, snapshot: result.rows[0], changedBy: userId });
  return result.rows[0];
}
...
export async function archiveTemplate(id: string, userId: string) {
  const pool = getPgPool();
  if (!pool) return { id };
  const result = await pool.query('UPDATE "delivery_templates" SET "active"=false,"archivedAt"=now(),"archivedBy"=$2,"updatedAt"=now() WHERE "id"=$1 AND "archivedAt" IS NULL RETURNING *', [id, userId]);
...
export async function listItems(projectId: string) {
  const pool = getPgPool();
  if (!pool) return [];
  const result = await pool.query('SELECT * FROM "delivery_items" WHERE "projectId"=$1 AND "archivedAt" IS NULL ORDER BY "sortOrder","sequenceNumber"', [projectId]);
  return result.rows;
}
...
export async function updateItem(id: string, patch: Record<string, unknown>) {
  const pool = getPgPool();
  if (!pool) return { id, ...patch };
  const allowed = new Set(["status", "responsibleId", "dueDate", "evidences", "payload", "title", "description"]);
...
const PREFIX: Record<string, string> = {
  activity: "ATV", bdcq: "BDCQ", workshop: "WS", dcd: "DCD", gap: "GAP",
  configuration: "CFG", unit_test: "TST", cycle_1: "TST", cycle_2: "TST",
...
export async function applyTrail(projectId: string, modules: string[], scopeItems: Array<{ id: string; key: string; module?: string }>, projectStartDate = "") {
  ...
  try {
    ...
    for (const entry of preview.items as any[]) {
      ...
      if (!entry.existing) {
        ...
        const dueDate = projectStartDate ? (() => {
          ...
          date.setUTCDate(date.getUTCDate() + Number(template.dueOffsetDays || 0));
    ...
    for (const entry of preview.items as any[]) {
      ...
      await client.query('UPDATE "delivery_items" SET "dependencyItemIds"=$2::jsonb WHERE "id"=$1', [itemId, JSON.stringify(dependencyItemIds)]);
    }
    await client.query("COMMIT");
    return { added, updated, preserved: preview.preserved, outOfScope: preview.outOfScope };
  } catch (error) {
...
export async function createRaid(projectId: string, input: any) {
  ...
  const sequenceResult = await pool.query(`SELECT nextval('"delivery_card_global_seq"')::bigint AS value`);
  const sequenceNumber = Number(sequenceResult.rows[0].value);
  const kind = input.kind === "issue" ? "issue" : "risk";
  const item = await insert("delivery_items", {
    id: `di_${nanoid(20)}`, code: `${PREFIX[kind]}-${String(sequenceNumber).padStart(6, "0")}`,
    sequenceNumber, projectId, type: kind, title: input.title, description: input.description || "",
    phase: input.phase || "Prepare", stage: "raid", module: input.module || "", scopeItemIds: input.scopeItemIds || [],
    required: input.required ?? false, ownerRole: "manager", responsibleId: input.responsibleId || "",
    dueDate: input.dueDate || "", status: input.status || "open", evidenceRequirements: [], evidences: input.attachments || [],
    approvalPolicy: input.approvalPolicy || { mode: "none", minimumApprovals: 1 }, payload: {},
  }) as any;
  await insert("delivery_raid_items", {
    id: `raid_${nanoid(20)}`, deliveryItemId: item.id, kind, category: input.category || "",
    cause: input.cause || "", consequence: input.consequence || "", probability: input.probability || 1,
    impact: input.impact || 1, severity: (input.probability || 1) * (input.impact || 1),
    strategy: input.strategy || "", responsePlan: input.responsePlan || "", workaround: input.workaround || "",
    rootCause: input.rootCause || "", sponsorId: input.sponsorId || "", nextAction: input.nextAction || "",
...
const ARCHIVABLE_TABLES = [
  "bdcq_answers", "bdcq_questions", "workshops", "dcd_documents", "gaps",
  "configurations", "workflow_test_cases", "activities", "workflow_bdcq_templates",
  "workflow_configuration_templates", "workflow_workshop_templates", "activity_templates",
] as const;

/** Planner is the source of truth and must never participate in a TechMove reset. */
const PROTECTED_PLANNER_TABLES = new Set([
  "projects", "resources", "allocations", "phases", "absences", "lookups",
  "app_users", "project_memberships", "project_approval_policies",
]);

function assertPlannerIsProtected() {
  const forbidden = ARCHIVABLE_TABLES.filter(table => PROTECTED_PLANNER_TABLES.has(table));
  if (forbidden.length)
    throw new Error(`Arquivamento bloqueado: tabelas do Planner detectadas (${forbidden.join(", ")})`);
}
...
export async function previewInitialArchive() {
  ...
  for (const table of ARCHIVABLE_TABLES) {
    const projectFilter = ["workflow_bdcq_templates", "workflow_configuration_templates", "workflow_workshop_templates", "activity_templates"].includes(table) ? "" : "";
    const result = await pool.query(`SELECT COUNT(*)::int AS count FROM "${table}" WHERE "archivedAt" IS NULL${projectFilter}`);
    tables[table] = Number(result.rows[0]?.count || 0);
  }
  return { total: Object.values(tables).reduce((sum, count) => sum + count, 0), tables };
}
...
export async function archiveInitialData(userId: string, reason: string) {
  ...
  try {
    await client.query("BEGIN");
    await client.query('INSERT INTO "delivery_archive_batches" ("id","reason","summary","createdBy") VALUES ($1,$2,$3::jsonb,$4)', [batchId, reason, "{}", userId]);
    for (const table of ARCHIVABLE_TABLES) {
      const rows = await client.query(`SELECT * FROM "${table}" WHERE "archivedAt" IS NULL FOR UPDATE`);
      summary[table] = rows.rowCount || 0;
      for (const row of rows.rows) {
        await client.query('INSERT INTO "delivery_archive_records" ("id","batchId","tableName","recordId","snapshot") VALUES ($1,$2,$3,$4,$5::jsonb)', [`dar_${nanoid(20)}`, batchId, table, String(row.id), JSON.stringify(row)]);
      }
      await client.query(`UPDATE "${table}" SET "archivedAt"=now() WHERE "archivedAt" IS NULL`);
      if (["workflow_bdcq_templates", "workflow_configuration_templates", "workflow_workshop_templates", "activity_templates"].includes(table))
        await client.query(`UPDATE "${table}" SET "active"=false WHERE "archivedAt" IS NOT NULL`);
    }
    await client.query('UPDATE "delivery_archive_batches" SET "summary"=$2::jsonb WHERE "id"=$1', [batchId, JSON.stringify(summary)]);
    await client.query("COMMIT");
    return { id: batchId, total: Object.values(summary).reduce((sum, count) => sum + count, 0), tables: summary };
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
  } finally {
...
export async function listArchiveBatches() {
  const pool = getPgPool();
  if (!pool) return [];
  const result = await pool.query('SELECT * FROM "delivery_archive_batches" ORDER BY "createdAt" DESC');
  return result.rows;
}
...
export async function restoreArchiveBatch(batchId: string, userId: string) {
  ...
  try {
    await client.query("BEGIN");
    const batch = await client.query('SELECT * FROM "delivery_archive_batches" WHERE "id"=$1 AND "restoredAt" IS NULL FOR UPDATE', [batchId]);
    if (!batch.rows[0]) throw new Error("Lote de arquivamento não encontrado ou já restaurado");
    const records = await client.query('SELECT "tableName","recordId","snapshot" FROM "delivery_archive_records" WHERE "batchId"=$1', [batchId]);
    for (const record of records.rows) {
      if (!(ARCHIVABLE_TABLES as readonly string[]).includes(record.tableName)) continue;
      ...
      await client.query(
        ...
        activeValue !== undefined ? [record.recordId, activeValue] : [record.recordId]
      );
    }
    await client.query('UPDATE "delivery_archive_batches" SET "restoredAt"=now(),"restoredBy"=$2 WHERE "id"=$1', [batchId, userId]);
    await client.query("COMMIT");
