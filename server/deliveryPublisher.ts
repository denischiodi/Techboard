...
async function stageIsComplete(projectId: string, type: string, stage: string) {
  ...
  return (
    ...
    Number(row.done || 0) >= Number(row.total || 0)
  );
}
...
function activityOccurrence(template: any, occurrence: any) {
  ...
  if (payload.recurrence === "weekly") {
    ...
    target.setUTCDate(
      target.getUTCDate() + ((Number(payload.weekday ?? 1) + 6) % 7)
    );
    return {
      ...occurrence,
      key: `${occurrence.key}|week-${period}`,
  ...
  if (payload.recurrence === "monthly") {
    ...
    return {
      ...occurrence,
      key: `${occurrence.key}|month-${period}`,
...
async function materializeOperational(
  template: any,
  project: any,
  occurrence: any,
  existing: any
  ...
  const initialOnly = async (
    ...
    const result = await pool.query(
      `SELECT "status" FROM "${table}" WHERE "id"=$1 AND "archivedAt" IS NULL`,
      [id]
    );
    return (
      !result.rows[0] || initialStatuses.includes(String(result.rows[0].status))
    );
  };
  const legacyEquivalent = async (
    table: string,
    textColumn: string,
    textValue: string,
    moduleValue = module
    ...
    const result = await pool.query(
      ...
      supportsModule
        ? [
            project.id,
            textValue,
            moduleValue || (table === "bdcq_questions" ? "Geral" : ""),
          ]
        : [project.id, textValue]
    );
    if (result.rows.length > 1)
      throw new Error(
        `Mais de um registro legado corresponde ao padrão em ${table}; revisão administrativa necessária`
      );
  ...
  if (template.type === "bdcq") {
    if (!existing) {
      const legacyId = await legacyEquivalent(
        "bdcq_questions",
        "question",
        String(payload.question || template.title)
      );
    ...
    if (existing?.targetId) {
      ...
      const answers = await pool?.query(
        `SELECT 1 FROM "bdcq_answers" WHERE "questionId"=$1 AND "archivedAt" IS NULL LIMIT 1`,
        [existing.targetId]
      );
      if (answers?.rowCount)
        return { id: existing.targetId, state: "preserved" };
      await workflowDb.updateBdcqQuestion(existing.targetId, {
        question: String(payload.question || template.title),
        category: String(payload.category || ""),
        module: module || "Geral",
        scopeItemIds: occurrence.scopeItemIds || [],
  ...
  if (template.type === "workshop") {
    ...
    if (existing?.targetId) {
      ...
      await workflowDb.updateWorkshop(existing.targetId, {
        ...
        module,
        scopeItemIds: occurrence.scopeItemIds || [],
        objective: payload.objective || template.description,
        content: payload.content || template.instructions,
  ...
  if (template.type === "configuration") {
    ...
    if (existing?.targetId) {
      if (
        !(await initialOnly("configurations", existing.targetId, ["Pendente"]))
      )
        return { id: existing.targetId, state: "preserved" };
      await workflowDb.updateConfiguration(existing.targetId, {
        module,
        category: payload.category || "Configuração",
  ...
  if (["unit_test", "cycle_1", "cycle_2"].includes(template.type)) {
    ...
    if (existing?.targetId) {
      if (
        !(await initialOnly("workflow_test_cases", existing.targetId, [
          "Não iniciado",
        ]))
      )
        return { id: existing.targetId, state: "preserved" };
      await workflowDb.updateWorkflowTestCase(existing.targetId, {
        type: testType(template.type),
        title: template.title,
  ...
  if (template.type === "activity") {
      ...
      users.find(
        user =>
          ...
          [user.name, user.email].some(
            value => normalize(value) === managerName
          )
      ) || users.find(user => user.active && user.role === "admin");
    if (!creator)
      throw new Error(
        "Projeto sem GP ou administrador ativo para criar a atividade"
      );
    ...
    const deliveryItem: any = (await deliveryStore.listItems(project.id)).find(
      item =>
        item.templateId === template.id && item.occurrenceKey === occurrence.key
    );
    if (!deliveryItem)
      throw new Error("A atividade padrão não foi materializada na Trilha");
    const sourceKey = deliveryItem.id;
    const current: any = await activityStore.findBySource(
      "delivery_template",
      sourceKey
    );
    ...
    await activityStore.upsertSourceActivity({
      ...
      description: [template.description, template.instructions]
        .filter(Boolean)
        .join("\n"),
      status: current?.status || "A fazer",
      priority: payload.priority || "Média",
      assigneeUserId: current?.assigneeUserId || creator.id,
      creatorUserId: creator.id,
      participantUserIds: current?.participantUserIds || [creator.id],
      dueDate:
        deliveryItem.dueDate ||
        occurrence.dueDate ||
        dueDate(project.startDate, template.dueOffsetDays),
...
async function publishTemplate(
  template: any,
  options: { confirmedProjectId?: string } = {}
) {
  ...
  if (
    !template.active ||
    template.archivedAt ||
    (template.effectiveFrom && template.effectiveFrom > today)
  )
    return summary;
  ...
  for (const project of projects as any[]) {
    ...
    if (
      ...
      (options.confirmedProjectId && project.id !== options.confirmedProjectId)
    ) {
      summary.outOfScope++;
    ...
    try {
      ...
      if (template.type === "activity")
        occurrences = occurrences.map(occurrence =>
          activityOccurrence(template, occurrence)
        );
      if (!occurrences.length) {
        summary.outOfScope++;
        projectResult.status = "out_of_scope";
        projectResult.message =
          "Módulo, scope item ou restrição de projeto não corresponde";
        summary.projects.push(projectResult);
        continue;
      }
      summary.applicable++;
      for (const occurrence of occurrences as any[]) {
        ...
        const completed = await stageIsComplete(
          project.id,
          template.type,
          template.stage
        );
        if (completed && options.confirmedProjectId !== project.id) {
          await saveMaterialization({
            ...
            projectId: project.id,
            occurrenceKey: occurrence.key,
            targetType: typeToTarget(template.type),
            state: "blocked",
            reason: "Etapa 100% concluída",
          });
          summary.blocked++;
          projectResult.blocked++;
          continue;
        }
        const result = await materializeOperational(
          template,
          project,
          occurrence,
          existing
        ...
        await saveMaterialization({
          ...
          targetType: typeToTarget(template.type),
          targetId: result.id,
          state: result.state === "preserved" ? "customized" : "current",
          reason:
            result.state === "preserved" ? "Execução local preservada" : "",
      ...
      projectResult.status = projectResult.created
        ? "created"
        : projectResult.updated
          ? "updated"
          : projectResult.blocked
...
export async function enqueueTemplatePublication(
  template: any,
  createdBy: string,
...
export async function cancelTemplatePublications(templateId: string) {
  ...
  const result = await pool.query(
    `UPDATE "delivery_publication_jobs" SET "status"='cancelled',"finishedAt"=now(),"updatedAt"=now()
     WHERE "templateId"=$1 AND "status"='pending'`,
    [templateId]
  );
...
export async function processPublicationJob(id: string) {
  ...
  const claimed = await pool.query(
    `UPDATE "delivery_publication_jobs" SET "status"='processing',"attempts"="attempts"+1,
      "startedAt"=now(),"updatedAt"=now() WHERE "id"=$1 AND "status" IN ('pending','failed') RETURNING *`,
    [id]
  );
  if (!claimed.rows[0]) return null;
  try {
    const template = await deliveryStore.getTemplate(
      claimed.rows[0].templateId
    );
    if (!template?.active || template?.archivedAt)
      throw new Error("Padrão inativo ou arquivado");
...
export async function retryPublicationJob(id: string) {
  ...
  if (!reset.rowCount)
    throw new Error("Esta publicação ainda está em processamento");
...
export async function resumePendingPublications() {
  ...
  for (const row of result.rows) {
    if (row.status === "processing")
      await pool.query(
        `UPDATE "delivery_publication_jobs" SET "status"='pending' WHERE "id"=$1`,
        [row.id]
      );
...
export async function enqueueReconciliation(createdBy = "system") {
  ...
  for (const template of templates.filter(
    ...
    if (await enqueueTemplatePublication(template, createdBy, "reconciliation"))
      queued++;
  }
  return queued;
}
...
export async function listBlocked(projectId: string) {
  ...
  const result = await pool.query(
    `SELECT m.*,t."title",t."type",t."stage",t."version"
     ...
     WHERE m."projectId"=$1 AND m."state"='blocked' ORDER BY t."stage",t."sortOrder",t."title"`,
    [projectId]
  );
  return result.rows;
}
...
export async function confirmBlocked(projectId: string, templateIds: string[]) {
  const templates = await Promise.all(
    templateIds.map(id => deliveryStore.getTemplate(id))
  );
  const summaries = [];
  for (const template of templates.filter(Boolean))
    summaries.push(
      await publishTemplate(template, { confirmedProjectId: projectId })
    );
