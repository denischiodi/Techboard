...
export const sapLibraryRouter = router({
  ...
  createScope: adminProcedure
    .input(
      z.object({
        code: z
          .string()
          .trim()
          .regex(/^[A-Za-z0-9][A-Za-z0-9_-]{1,127}$/),
        name: z.string().trim().min(1).max(512),
        summary: z.string().trim().max(10000).default(""),
        module: z.string().trim().max(128).default(""),
        processArea: z.string().trim().max(256).default(""),
        fileName: z
          .string()
          .trim()
          .max(512)
    ...
    .input(
      z.object({
        ...
        data: z.object({
          ...
          module: z.string().trim().max(128).optional(),
          processArea: z.string().trim().max(256).optional(),
        }),
      })
    )
    .mutation(({ input }) => library.updateScope(input.id, input.data)),
  addWordAsset: adminProcedure
    .input(
      z.object({
  ...
  prepareUpload: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      return storagePresignPut(
        `sap-library/${input.releaseCode}/${nanoid()}-${safeName}`
      );
    }),
  ...
  activate: adminProcedure
    ...
    .mutation(({ ctx, input }) =>
      library.activateRelease(input.id, ctx.appUser.id)
    ),
