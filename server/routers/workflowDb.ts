...
async function insertRow<T extends object>(table: string, data: T, jsonColumns: string[] = []) {
  const pool = getPgPool();
  if (!pool) return data;
  const entries = Object.entries(data as Record<string, unknown>).filter(([, value]) => value !== undefined && value !== null);
  const columns = entries.map(([key]) => quoteIdentifier(key)).join(", ");
  const placeholders = entries.map((_, index) => `$${index + 1}`).join(", ");
  const values = entries.map(([key, value]) => jsonColumns.includes(key) ? JSON.stringify(value) : value);
  const result = await pool.query(`INSERT INTO ${quoteIdentifier(table)} (${columns}) VALUES (${placeholders}) RETURNING *`, values);
  return result.rows[0] as T;
}

async function updateRow(table: string, id: string, data: object, allowedColumns: string[], jsonColumns: string[] = []) {
  const pool = getPgPool();
  if (!pool) return;
  const allowed = new Set(allowedColumns);
  const entries = Object.entries(data as Record<string, unknown>).filter(([key, value]) => allowed.has(key) && value !== undefined);
  if (!entries.length) return;
  const assignments = entries.map(([key], index) => `${quoteIdentifier(key)} = $${index + 2}`);
  const values = entries.map(([key, value]) => jsonColumns.includes(key) ? JSON.stringify(value) : value);
  await pool.query(`UPDATE ${quoteIdentifier(table)} SET ${assignments.join(", ")}, "updatedAt" = now() WHERE "id" = $1`, [id, ...values]);
}
...
async function bulkUpdateRows(table: string, ids: string[], data: object, allowedColumns: string[]) {
  const pool = getPgPool();
  ...
  const result = await pool.query(`UPDATE ${quoteIdentifier(table)} SET ${assignments.join(", ")}, "updatedAt" = now() WHERE "id" = ANY($${entries.length + 1}::varchar[])`, [...entries.map(([, value]) => value), ids]);
...
export async function getWorkflowEntityProjectId(table: string, id: string) {
  ...
  if (directProjectTables.has(table)) {
    const result = await pool.query(`SELECT "projectId" FROM ${quoteIdentifier(table)} WHERE "id" = $1 LIMIT 1`, [id]);
...
export async function createClientRequirement(data: typeof clientRequirements.$inferInsert) {
  return insertRow("client_requirements", data);
}

const requirementUpdateColumns = new Set(["code", "title", "description", "module", "category", "priority", "status", "source", "acceptanceCriteria", "responsible"]);

export async function updateClientRequirement(id: string, data: Partial<typeof clientRequirements.$inferInsert>) {
  return updateRow("client_requirements", id, data, Array.from(requirementUpdateColumns));
}
...
export async function updateScopeItem(id: string, data: Partial<typeof scopeItems.$inferInsert>) {
  return updateRow("scope_items", id, data, ["module", "code", "name", "processArea", "description", "active"]);
}
...
export async function getBdcqAnswerByQuestion(projectId: string, questionId: string) {
  const pool = getPgPool();
  if (!pool) return null;
  const result = await pool.query(`SELECT * FROM "bdcq_answers" WHERE "projectId" = $1 AND "questionId" = $2 ORDER BY "updatedAt" DESC LIMIT 1`, [projectId, questionId]);
...
export async function updateBdcqAnswer(id: string, data: Partial<typeof bdcqAnswers.$inferInsert>) {
  return updateRow("bdcq_answers", id, data, ["answer", "answeredBy", "attachments"], ["attachments"]);
}
export async function updateBdcqAnswerWithHistory(id: string, data: Partial<typeof bdcqAnswers.$inferInsert>, historyId: string, changedBy: string) {
  const pool = getPgPool();
  if (!pool) return { id, ...data };
  const client = await pool.connect();
  try {
    await client.query("BEGIN");
    const currentResult = await client.query(`SELECT * FROM "bdcq_answers" WHERE "id" = $1 FOR UPDATE`, [id]);
    const current = currentResult.rows[0] as typeof bdcqAnswers.$inferSelect | undefined;
    if (!current) throw new Error("Resposta BDCQ não encontrada");
    const changed = (data.answer !== undefined && data.answer !== current.answer) || (data.answeredBy !== undefined && data.answeredBy !== current.answeredBy) || data.attachments !== undefined;
    if (!changed) { await client.query("COMMIT"); return current; }
    await client.query(`INSERT INTO "bdcq_answer_history" ("id", "answerId", "questionId", "projectId", "answer", "answeredBy", "changedBy") VALUES ($1,$2,$3,$4,$5,$6,$7)`, [historyId, id, current.questionId, current.projectId, current.answer, current.answeredBy, changedBy]);
    const answer = data.answer ?? current.answer;
    const answeredBy = data.answeredBy ?? current.answeredBy;
    const attachments = data.attachments === undefined ? current.attachments : data.attachments;
    const result = await client.query(`UPDATE "bdcq_answers" SET "answer" = $2, "answeredBy" = $3, "attachments" = $4::jsonb, "updatedAt" = now() WHERE "id" = $1 RETURNING *`, [id, answer, answeredBy, JSON.stringify(attachments || [])]);
...
export async function listBdcqAnswerHistory(answerId: string) {
  const pool = getPgPool();
  if (!pool) return [];
  const result = await pool.query(`SELECT * FROM "bdcq_answer_history" WHERE "answerId" = $1 ORDER BY "createdAt" DESC`, [answerId]);
...
export async function updateWorkshop(id: string, data: Partial<typeof workshops.$inferInsert>) {
  return updateRow("workshops", id, data, ["title", "module", "scheduledDate", "duration", "participants", "agenda", "status", "notes"], ["participants", "agenda"]);
}
...
export async function listMinutesByProject(projectId: string) {
  const pool = getPgPool();
  if (!pool) return [];
  const result = await pool.query(`SELECT m.* FROM "meeting_minutes" m JOIN "workshops" w ON w."id" = m."workshopId" WHERE w."projectId" = $1`, [projectId]);
...
export async function updateMinutes(id: string, data: Partial<typeof meetingMinutes.$inferInsert>) {
  return updateRow("meeting_minutes", id, data, ["content", "generatedBy", "version"]);
}
...
export async function findDcdBySourceHash(projectId: string, sourceHash: string) {
  const pool = getPgPool();
  if (!pool) return null;
  const result = await pool.query(`SELECT * FROM "dcd_documents" WHERE "projectId" = $1 AND "sourceHash" = $2 ORDER BY "createdAt" DESC LIMIT 1`, [projectId, sourceHash]);
  return (result.rows[0] as typeof dcdDocuments.$inferSelect | undefined) || null;
}
export async function getLatestDcdByModule(projectId: string, module: string) {
  const pool = getPgPool();
  if (!pool) return null;
  const result = await pool.query(`SELECT * FROM "dcd_documents" WHERE "projectId" = $1 AND "module" = $2 ORDER BY "version" DESC, "createdAt" DESC LIMIT 1`, [projectId, module]);
  return (result.rows[0] as typeof dcdDocuments.$inferSelect | undefined) || null;
}
...
export async function updateDcdDocument(id: string, data: Partial<typeof dcdDocuments.$inferInsert>) {
  return updateRow("dcd_documents", id, data, ["seriesId", "sourceHash", "module", "title", "content", "version", "status"]);
}
...
export async function updateGap(id: string, data: Partial<typeof gaps.$inferInsert>) {
  return updateRow("gaps", id, data, ["dcdId", "module", "description", "impact", "responsible", "resolution", "status"]);
}
export async function deleteGap(id: string) {
  return deleteRow("gaps", id);
}
export async function bulkUpdateGaps(ids: string[], data: Partial<typeof gaps.$inferInsert>) {
...
export async function updateConfiguration(id: string, data: Partial<typeof configurations.$inferInsert>) {
  return updateRow("configurations", id, data, ["module", "category", "description", "responsible", "status", "notes"]);
}
...
// ===== Workflow Audit Log =====
export async function createWorkflowAudit(data: typeof workflowAuditLog.$inferInsert) {
  return insertRow("workflow_audit_log", data, ["details"]);
}
export async function listWorkflowAudit(projectId: string, limit = 100) {
  const pool = getPgPool();
  if (!pool) return [];
  const result = await pool.query(`SELECT * FROM "workflow_audit_log" WHERE "projectId" = $1 ORDER BY "createdAt" DESC LIMIT $2`, [projectId, limit]);
...
export async function listWorkflowPrompts() {
  const pool = getPgPool();
  if (!pool) return [];
  const result = await pool.query(`SELECT * FROM "workflow_prompts" ORDER BY "name" ASC`);
...
export async function getWorkflowPrompt(key: string) {
  const pool = getPgPool();
  if (!pool) return null;
  const result = await pool.query(`SELECT * FROM "workflow_prompts" WHERE "key" = $1 LIMIT 1`, [key]);
  return (result.rows[0] as typeof workflowPrompts.$inferSelect | undefined) || null;
}

export async function upsertWorkflowPrompt(data: typeof workflowPrompts.$inferInsert) {
  const pool = getPgPool();
  if (!pool) return data;
  const result = await pool.query(
    `INSERT INTO "workflow_prompts" ("key", "name", "description", "systemPrompt", "model", "updatedBy") VALUES ($1, $2, $3, $4, $5, $6)
     ...
     RETURNING *`,
    [data.key, data.name, data.description || null, data.systemPrompt, data.model || "", data.updatedBy || ""],
  );
