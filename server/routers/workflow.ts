...
const legacyKey = (...values: unknown[]) => values.map(value => String(value || "").trim().toLocaleLowerCase("pt-BR")).join("|");

async function importLegacyTechMove(projectId: string, legacy: TechMoveData) {
  const [currentScope, currentQuestions, currentAnswers, currentWorkshops, currentDcds, currentGaps, currentConfigurations] = await Promise.all([
    wdb.listScopeItems(projectId), wdb.listBdcqQuestions(projectId), wdb.listBdcqAnswers(projectId),
    wdb.listWorkshops(projectId), wdb.listDcdDocuments(projectId, true), wdb.listGaps(projectId), wdb.listConfigurations(projectId),
  ]);
  const imported = { scopeItems: 0, questions: 0, answers: 0, workshops: 0, transcripts: 0, minutes: 0, dcdDocuments: 0, gaps: 0, configurations: 0, ignored: 0 };
  const scopeKeys = new Set(currentScope.map((item: any) => legacyKey(item.module, item.code, item.name)));
  const scopeByCode = new Map(currentScope.filter((item: any) => item.code).map((item: any) => [legacyKey(item.code), item.id]));
  for (const item of legacy.scopeItems) {
    const key = legacyKey(item.module, item.code, item.name);
    if (scopeKeys.has(key)) { imported.ignored++; continue; }
    const id = nanoid();
    await wdb.createScopeItem({ id, projectId, module: item.module, code: item.code || "", name: item.name, processArea: item.processArea || "", description: item.description, active: item.active ? 1 : 0 });
    scopeKeys.add(key); if (item.code) scopeByCode.set(legacyKey(item.code), id); imported.scopeItems++;
  }

  const questionKeys = new Set(currentQuestions.map((item: any) => legacyKey(item.module, item.question)));
  const answerQuestionIds = new Set(currentAnswers.map((item: any) => item.questionId));
  for (const question of legacy.questions) {
    const key = legacyKey(question.module, question.text);
    let id = currentQuestions.find((item: any) => legacyKey(item.module, item.question) === key)?.id as string | undefined;
    if (!questionKeys.has(key)) {
      id = nanoid();
      const scopeItemIds = question.scopeItemCodes.map(code => scopeByCode.get(legacyKey(code))).filter((value): value is string => Boolean(value));
      await wdb.createBdcqQuestion({ id, projectId, module: question.module, category: question.category || "", question: question.text, scopeItemIds, isDefault: question.reusable ? 1 : 0, sortOrder: currentQuestions.length + imported.questions });
      questionKeys.add(key); imported.questions++;
    } else imported.ignored++;
    if (!id) continue;
    if (question.answer.trim() && !answerQuestionIds.has(id)) {
      await wdb.createBdcqAnswer({ id: nanoid(), projectId, questionId: id, answer: question.answer, answeredBy: question.ownerRole || "Importação TechMove", attachments: question.evidence ? [question.evidence] : [] });
      answerQuestionIds.add(id); imported.answers++;
    }
  }

  const workshopKeys = new Set(currentWorkshops.map((item: any) => legacyKey(item.module, item.title, item.scheduledDate)));
  for (const workshop of legacy.workshops) {
    const key = legacyKey(workshop.module, workshop.title, workshop.date);
    if (workshopKeys.has(key)) { imported.ignored++; continue; }
    const id = nanoid();
    const participants = workshop.participants.split(/[;,\n]/).map(value => value.trim()).filter(Boolean);
    await wdb.createWorkshop({ id, projectId, title: workshop.title, module: workshop.module, scheduledDate: workshop.date || "", duration: workshop.durationMinutes ? `${workshop.durationMinutes} min` : "", participants, agenda: workshop.script ? workshop.script.split("\n").filter(Boolean) : [], status: workshop.completed ? "Concluído" : "Planejado", notes: workshop.decisions || "" });
    if (workshop.transcript.trim()) { await wdb.createTranscript({ id: nanoid(), workshopId: id, content: workshop.transcript, uploadedBy: "Importação TechMove" }); imported.transcripts++; }
    if ((workshop.minutes || "").trim()) { await wdb.createMinutes({ id: nanoid(), workshopId: id, content: workshop.minutes!, generatedBy: "legacy" }); imported.minutes++; }
    workshopKeys.add(key); imported.workshops++;
  }

  if (legacy.dcdDraft.trim() && !currentDcds.some((item: any) => legacyKey(item.content) === legacyKey(legacy.dcdDraft))) {
    const id = nanoid();
    await wdb.createDcdDocument({ id, seriesId: id, projectId, title: "DCD importado do TechMove legado", content: legacy.dcdDraft, module: "", status: "Rascunho" });
    imported.dcdDocuments++;
  }
  const gapKeys = new Set(currentGaps.map((item: any) => legacyKey(item.module, item.description)));
  for (const gap of legacy.gaps) {
    const description = gap.description || gap.title;
    const key = legacyKey(gap.module, description);
    if (gapKeys.has(key)) { imported.ignored++; continue; }
    const impact = gap.severity === "Critico" ? "Crítico" : gap.severity === "Medio" ? "Médio" : gap.severity;
    const status = gap.status === "Em analise" ? "Em Análise" : gap.status === "Aprovado" ? "Aceito" : gap.status === "Rejeitado" ? "Resolvido" : "Aberto";
    await wdb.createGap({ id: nanoid(), projectId, module: gap.module, description, impact, responsible: gap.assignedTo || "", resolution: gap.resolution, status });
    gapKeys.add(key); imported.gaps++;
  }
  const configurationKeys = new Set(currentConfigurations.map((item: any) => legacyKey(item.module, item.description)));
  for (const configuration of legacy.configurations || []) {
    ...
    if (configurationKeys.has(key)) { imported.ignored++; continue; }
    const status = configuration.status === "Em andamento" ? "Em andamento" : configuration.status === "Concluido" ? "Concluído" : configuration.status;
    await wdb.createConfiguration({ id: nanoid(), projectId, module: configuration.module, category: configuration.path || "", description, responsible: configuration.owner || "", status, notes: `Prioridade: ${configuration.priority}` });
    configurationKeys.add(key); imported.configurations++;
  }
  return imported;
}
...
export async function ensureBdcqTemplates(projectId: string, modules?: string[]) {
  const [existing, scopeItems, customTemplates] = await Promise.all([
    wdb.listBdcqQuestions(projectId), wdb.listScopeItems(projectId), wdb.listBdcqTemplateLibrary(),
  ]);
  const normalize = (value: string) => value.trim().toLocaleLowerCase("pt-BR");
  const existingQuestions = new Set(existing.map((question: any) => `${normalize(question.module || "Geral")}:${normalize(question.question)}`));
  const moduleSet = modules?.length ? new Set(modules.map(module => module.trim().toUpperCase())) : null;
  const templates: Array<{ id?: string; module: string; category: string; question: string; scopeItemIds?: string[] }> = BDCQ_TEMPLATES
    .filter(template => !moduleSet || moduleSet.has(template.module))
    .map(template => ({ ...template }));
  for (const template of customTemplates.filter((item: any) => item.active !== 0)) {
    const templateModules = (template.modules || []).map((module: string) => module.trim().toUpperCase());
    const keys = new Set((template.scopeItemKeys || []).map((key: string) => normalize(key)));
    const matchedScope = scopeItems.filter((item: any) => keys.has(normalize(item.code || item.name)));
    const relevantModules = new Set<string>();
    templateModules.forEach((module: string) => { if (!moduleSet || moduleSet.has(module)) relevantModules.add(module); });
    matchedScope.forEach((item: any) => { const module = String(item.module || "Geral").toUpperCase(); if (!moduleSet || moduleSet.has(module)) relevantModules.add(module); });
    if (!templateModules.length && !keys.size) relevantModules.add("Geral");
    for (const module of relevantModules) templates.push({ id: template.id, module, category: template.category || "", question: template.question, scopeItemIds: matchedScope.filter((item: any) => String(item.module || "Geral").toUpperCase() === module || !templateModules.length).map((item: any) => item.id) });
  }
  let added = 0;
  for (const template of templates) {
    const key = `${normalize(template.module)}:${normalize(template.question)}`;
    if (existingQuestions.has(key)) continue;
    await wdb.createBdcqQuestion({ id: nanoid(), projectId, module: template.module, category: template.category, question: template.question, templateId: template.id || "", scopeItemIds: template.scopeItemIds || [], isDefault: 1, sortOrder: existing.length + added });
...
async function safeWorkflowNotification(projectId: string, title: string, message: string) {
  try {
    const project = await plannerStore.getProjectById(projectId);
    const context = project ? `Projeto: ${project.name}\nCliente: ${project.client || "-"}\nGestor: ${project.manager || "-"}\n\n` : "";
...
async function notifyIfBdcqModuleCompleted(projectId: string, questionId: string) {
  const [questions, answers] = await Promise.all([wdb.listBdcqQuestions(projectId), wdb.listBdcqAnswers(projectId)]);
  const answeredQuestion = questions.find((question: any) => question.id === questionId);
  if (!answeredQuestion) return false;
  const moduleQuestions = questions.filter((question: any) => question.module === answeredQuestion.module);
  const answeredIds = new Set(answers.filter((answer: any) => String(answer.answer || "").trim()).map((answer: any) => answer.questionId));
  if (!moduleQuestions.length || !moduleQuestions.every((question: any) => answeredIds.has(question.id))) return false;
  return safeWorkflowNotification(projectId, `BDCQ ${answeredQuestion.module} concluído`, `Todas as ${moduleQuestions.length} perguntas do módulo ${answeredQuestion.module} foram respondidas. A etapa está pronta para revisão e continuidade do Workflow.`);
}
...
export const workflowRouter = router({
  prompts: router({
    ...
    update: adminProcedure.input(z.object({
      key: z.enum(["agenda_suggestion", "minutes_generation", "dcd_generation", "dcd_refinement", "gaps_extraction", "configurations_extraction"]),
  ...
  reports: router({
    consolidatedPdf: workflowProjectProcedure().input(z.object({ projectId: z.string().min(1) })).mutation(async ({ ctx, input }) => {
      const [project, requirements, dcds, gaps, configurations, testCases] = await Promise.all([
        plannerStore.getProjectById(input.projectId), wdb.listClientRequirements(input.projectId),
        wdb.listDcdDocuments(input.projectId, true), wdb.listGaps(input.projectId), wdb.listConfigurations(input.projectId), wdb.listWorkflowTestCases(input.projectId),
      ]);
      if (!project) throw new TRPCError({ code: "NOT_FOUND", message: "Projeto não encontrado" });
      const markdown = buildWorkflowConsolidatedMarkdown({ project, requirements, dcds, gaps, configurations, testCases });
      const pdf = generateWorkflowPdf(`TechMove - ${project.name}`, markdown);
      const filename = `workflow-${project.name.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").toLowerCase() || "projeto"}.pdf`;
      await recordWorkflowAudit(ctx, input.projectId, "WORKFLOW_REPORT_EXPORTED", "project", input.projectId, { filename, bytes: pdf.length, dcds: dcds.length, gaps: gaps.length, configurations: configurations.length, testCases: testCases.length, requirements: requirements.length });
  ...
  search: workflowProjectProcedure().input(z.object({
    ...
    const [scope, questions, answers, workshops, requirements, minutes, dcds, gaps, configs, testCases] = await Promise.all([
    ...
    add(scope, "Escopo", "/techmove/scope-items", item => item.name, item => `${item.code || ""} ${item.module || ""} ${item.description || ""}`);
    add(questions, "BDCQ", "/techmove/bdcq", item => item.question, item => `${item.module || ""} ${item.category || ""}`);
    add(answers, "Resposta BDCQ", "/techmove/bdcq", item => `Resposta da pergunta ${item.questionId}`, item => item.answer || "");
    add(workshops, "Workshop", "/techmove/workshops", item => item.title, item => `${item.module || ""} ${item.notes || ""} ${(item.agenda || []).join(" ")}`);
    add(requirements, "Requisito", "/techmove/workshops", item => item.title, item => `${item.description || ""} ${item.acceptanceCriteria || ""} ${item.module || ""}`);
    add(minutes, "Ata", "/techmove/workshops", item => item.title || "Ata de workshop", item => item.content || "");
    add(dcds, "DCD", "/techmove/dcd", item => item.title, item => `${item.module || ""} ${item.content || ""}`);
    add(gaps, "Gap", "/techmove/gaps", item => item.description, item => `${item.module || ""} ${item.resolution || ""} ${item.responsible || ""}`);
    add(configs, "Configuração", "/techmove/configurations", item => item.description, item => `${item.module || ""} ${item.category || ""} ${item.responsible || ""}`);
    add(testCases, "Caso de teste", "/techmove/tests", item => item.title, item => `${item.type || ""} ${item.code || ""} ${item.module || ""} ${item.expectedResult || ""} ${item.actualResult || ""}`);
  ...
  dashboard: protectedProcedure.query(async ({ ctx }) => {
    ...
    const stageCounts: Record<string, number> = { Escopo: 0, BDCQ: 0, Workshops: 0, DCD: 0, Gaps: 0, Configurações: 0, Testes: 0, Concluído: 0 };
    ...
    await Promise.all(projects.map(async project => {
      const [scope, questions, answers, workshops, requirements, minutes, dcds, gaps, configs, testCases] = await Promise.all([
        ...
        wdb.listDcdDocuments(project.id), wdb.listGaps(project.id), wdb.listConfigurations(project.id), wdb.listWorkflowTestCases(project.id),
      ]);
      if (![scope, questions, workshops, requirements, dcds, gaps, configs, testCases].some(items => items.length)) return;
      workflowsInProgress++;
      ...
      const stage = scope.length === 0 ? "Escopo" : pending.length > 0 ? "BDCQ" : workshops.some((item: any) => !["Realizado", "Concluído", "Cancelado"].includes(item.status)) ? "Workshops" : !dcds.some((item: any) => item.status === "Aprovado") ? "DCD" : openGaps.length ? "Gaps" : configs.some((item: any) => item.status !== "Concluído") ? "Configurações" : !testCases.length || testCases.some((item: any) => item.status !== "Aprovado") ? "Testes" : "Concluído";
      stageCounts[stage]++;
      pending.filter((question: any) => new Date(question.createdAt).getTime() < cutoff).slice(0, 5).forEach((question: any) => alerts.push({ type: "BDCQ", label: `Pergunta sem resposta: ${question.question}`, projectId: project.id, projectName: project.name, route: "/techmove/bdcq" }));
      const minuteWorkshopIds = new Set(minutes.map((minute: any) => minute.workshopId));
      workshops.filter((workshop: any) => ["Realizado", "Concluído"].includes(workshop.status) && !minuteWorkshopIds.has(workshop.id)).forEach((workshop: any) => alerts.push({ type: "Workshop", label: `Workshop sem ata: ${workshop.title}`, projectId: project.id, projectName: project.name, route: "/techmove/workshops" }));
      dcds.filter((dcd: any) => dcd.status === "Rascunho" && new Date(dcd.updatedAt).getTime() < draftCutoff).forEach((dcd: any) => alerts.push({ type: "DCD", label: `DCD em rascunho há mais de 14 dias: ${dcd.title}`, projectId: project.id, projectName: project.name, route: "/techmove/dcd" }));
      openGaps.filter((gap: any) => !gap.responsible).forEach((gap: any) => alerts.push({ type: "Gap", label: `Gap sem responsável: ${gap.description}`, projectId: project.id, projectName: project.name, route: "/techmove/gaps" }));
    }));
  ...
  projectIndicators: protectedProcedure.query(async ({ ctx }) => {
    const projects = await listWorkflowProjects(ctx.appUser);
    const indicators = await Promise.all(projects.map(async project => {
      const [scope, questions, answers, workshops, dcds, gaps, configurations, testCases] = await Promise.all([
        wdb.listScopeItems(project.id), wdb.listBdcqQuestions(project.id), wdb.listBdcqAnswers(project.id),
        wdb.listWorkshops(project.id), wdb.listDcdDocuments(project.id), wdb.listGaps(project.id), wdb.listConfigurations(project.id), wdb.listWorkflowTestCases(project.id),
      ]);
      const hasWorkflow = [scope, questions, workshops, dcds, gaps, configurations, testCases].some(items => items.length > 0);
      if (!hasWorkflow) return null;
      const answeredIds = new Set(answers.filter((answer: any) => String(answer.answer || "").trim()).map((answer: any) => answer.questionId));
      const openGaps = gaps.filter((gap: any) => !["Resolvido", "Aceito"].includes(gap.status));
      const stage = scope.length === 0 ? "Escopo" : questions.some((question: any) => !answeredIds.has(question.id)) ? "BDCQ" : workshops.some((item: any) => !["Realizado", "Concluído", "Cancelado"].includes(item.status)) ? "Workshops" : !dcds.some((item: any) => item.status === "Aprovado") ? "DCD" : openGaps.length ? "Gaps" : configurations.some((item: any) => item.status !== "Concluído") ? "Configurações" : !testCases.length || testCases.some((item: any) => item.status !== "Aprovado") ? "Testes" : "Concluído";
      return { projectId: project.id, stage };
    }));
    return indicators.filter((indicator): indicator is NonNullable<typeof indicator> => Boolean(indicator));
  }),
  progress: workflowProjectProcedure().input(z.object({ projectId: z.string() })).query(async ({ input }) => {
    const [scope, questions, answers, workshopList, requirements, documents, gapList, configList, testCases] = await Promise.all([
    ...
    const approvedTests = testCases.filter((item: any) => item.status === "Aprovado").length;
    return {
      steps: [
  ...
  legacy: router({
    preview: workflowProjectProcedure().input(z.object({ projectId: z.string().min(1) })).query(async ({ input }) => {
    ...
    import: workflowProjectProcedure(true).input(z.object({ projectId: z.string().min(1) })).mutation(async ({ ctx, input }) => {
      const legacy = await plannerStore.getTechMoveData(input.projectId);
      const imported = await importLegacyTechMove(input.projectId, legacy);
      await recordWorkflowAudit(ctx, input.projectId, "TECHMOVE_LEGACY_IMPORTED", "project", input.projectId, imported);
      return imported;
    }),
  }),
  requirements: router({
    list: workflowProjectProcedure().input(z.object({
      ...
      ...paginationInput,
    })).query(({ input }) => wdb.listClientRequirements(input.projectId, input.workshopId, input)),
  ...
  // ===== Scope Items =====
  scopeItems: router({
    list: workflowProjectProcedure().input(z.object({ projectId: z.string(), ...paginationInput })).query(({ input }) =>
      wdb.listScopeItems(input.projectId, input)
    ),
    create: workflowProjectProcedure(true).input(z.object({
      projectId: z.string(),
      module: z.string(),
  ...
  bdcq: router({
    templates: router({
      list: protectedProcedure.query(async () => {
        ...
        const builtIn = BDCQ_TEMPLATES.map((template, index) => ({ id: `builtin-${template.module}-${index}`, question: template.question, category: template.category, modules: [template.module], scopeItemKeys: [], active: 1, createdBy: "Sistema", builtIn: true }));
        return [...builtIn, ...custom.map(template => ({ ...template, builtIn: false }))];
      }),
      create: adminProcedure.input(z.object({
        question: z.string().trim().min(1).max(5000), category: z.string().trim().max(256).optional(),
        modules: z.array(z.string().trim().min(1).max(128)).max(30).default([]),
        scopeItemKeys: z.array(z.string().trim().min(1).max(512)).max(200).default([]), active: z.number().int().min(0).max(1).optional(),
      })).mutation(({ ctx, input }) => wdb.createBdcqTemplate({ id: nanoid(), ...input, category: input.category || "", active: input.active ?? 1, createdBy: ctx.appUser.name || ctx.appUser.email })),
      update: adminProcedure.input(z.object({
        id: z.string().min(1), data: z.object({ question: z.string().trim().min(1).max(5000).optional(), category: z.string().trim().max(256).optional(), modules: z.array(z.string().trim().min(1).max(128)).max(30).optional(), scopeItemKeys: z.array(z.string().trim().min(1).max(512)).max(200).optional(), active: z.number().int().min(0).max(1).optional() }),
      })).mutation(({ input }) => wdb.updateBdcqTemplate(input.id, input.data)),
      delete: adminProcedure.input(z.object({ id: z.string().min(1) })).mutation(({ input }) => wdb.deleteBdcqTemplate(input.id)),
      applyToProject: workflowProjectProcedure(true).input(z.object({ projectId: z.string(), modules: z.array(z.string()).optional() })).mutation(async ({ input }) => ({ added: await ensureBdcqTemplates(input.projectId, input.modules) })),
    }),
    questions: router({
      list: workflowProjectProcedure().input(z.object({ projectId: z.string(), ...paginationInput })).query(({ input }) =>
        wdb.listBdcqQuestions(input.projectId, input)
      ),
      create: workflowProjectProcedure(true).input(z.object({
        projectId: z.string(),
        module: z.string(),
        ...
        if (input.scopeItemIds?.length) await assertEntitiesBelongToProject("scope_items", input.scopeItemIds, input.projectId);
        const id = nanoid();
        return wdb.createBdcqQuestion({ id, projectId: input.projectId, module: input.module, question: input.question, category: input.category || "", scopeItemIds: input.scopeItemIds || [], isDefault: input.isDefault ?? 0, sortOrder: input.sortOrder ?? 0 });
      }),
    ...
    answers: router({
      list: workflowProjectProcedure().input(z.object({ projectId: z.string(), questionIds: z.array(z.string()).max(500).optional(), ...paginationInput })).query(({ input }) =>
        input.questionIds ? wdb.listBdcqAnswersForQuestions(input.projectId, input.questionIds) : wdb.listBdcqAnswers(input.projectId, input)
      ),
      create: workflowProjectProcedure(true).input(z.object({
        questionId: z.string(),
        projectId: z.string(),
        ...
        const questionProjectId = await wdb.getWorkflowEntityProjectId("bdcq_questions", input.questionId);
        if (questionProjectId !== input.projectId) throw new TRPCError({ code: "BAD_REQUEST", message: "A pergunta não pertence ao projeto selecionado" });
        ...
        const id = nanoid();
        const created = await wdb.createBdcqAnswer({ id, projectId: input.projectId, questionId: input.questionId, answer: input.answer, answeredBy: input.answeredBy || "", attachments: input.attachments || [] });
  ...
  workshops: router({
    list: workflowProjectProcedure().input(z.object({ projectId: z.string(), ...paginationInput })).query(({ input }) =>
      wdb.listWorkshops(input.projectId, input)
    ),
    create: workflowProjectProcedure(true).input(z.object({
      projectId: z.string(),
      title: z.string(),
...
Retorne a sugestão em formato markdown com workshops sugeridos, duração estimada e temas a cobrir.`;
    ...
    transcripts: router({
      list: workflowEntityProcedure("workshops", false, "workshopId").input(z.object({ workshopId: z.string(), ...paginationInput })).query(({ input }) =>
        wdb.listTranscripts(input.workshopId, input)
      ),
      ...
      transcribe: workflowEntityProcedure("workshops", true, "workshopId").input(z.object({
        workshopId: z.string(), fileName: z.string().trim().min(1).max(255),
        contentType: z.enum(["audio/mpeg", "audio/mp3", "audio/wav", "audio/wave", "audio/webm", "audio/ogg", "audio/mp4", "audio/m4a"]),
        base64: z.string().min(1), language: z.string().max(10).optional(),
      })).mutation(async ({ ctx, input }) => {
        const buffer = Buffer.from(input.base64, "base64");
        if (!buffer.length) throw new TRPCError({ code: "BAD_REQUEST", message: "Arquivo de áudio vazio ou inválido" });
        if (buffer.length > 16 * 1024 * 1024) throw new TRPCError({ code: "PAYLOAD_TOO_LARGE", message: "O áudio deve ter no máximo 16 MB" });
        const safeName = input.fileName.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9._-]+/g, "-");
        const stored = await storagePut(`workflow/workshops/${input.workshopId}/${safeName}`, buffer, input.contentType);
        const audioUrl = await storageGetSignedUrl(stored.key);
        const result = await transcribeAudio({ audioUrl, language: input.language || "pt", prompt: "Transcreva integralmente este workshop SAP em português, preservando nomes, módulos, decisões e responsáveis." });
        if ("error" in result) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: `${result.error}${result.details ? `: ${result.details}` : ""}` });
        const id = nanoid();
        const transcript = await wdb.createTranscript({ id, workshopId: input.workshopId, content: result.text, fileUrl: stored.url, uploadedBy: ctx.user.name || ctx.user.email || "Usuário" });
        const projectId = await wdb.getWorkflowEntityProjectId("workshops", input.workshopId);
        if (projectId) await recordWorkflowAudit(ctx, projectId, "WORKSHOP_AUDIO_TRANSCRIBED", "workshop", input.workshopId, { transcriptId: id, fileName: input.fileName, duration: result.duration, language: result.language });
        return { transcript, language: result.language, duration: result.duration };
      }),
...
Retorne em formato markdown.`;
  ...
  dcd: router({
    ...
    update: workflowEntityProcedure("dcd_documents", true).input(z.object({
      ...
      const projectId = before?.projectId || await wdb.getWorkflowEntityProjectId("dcd_documents", input.id);
      ...
      if (projectId && input.data.status === "Aprovado" && before?.status !== "Aprovado") {
        await safeWorkflowNotification(projectId, "DCD aprovado", `${input.data.title || before?.title || "DCD"} foi aprovado por ${ctx.user.name || ctx.user.email || "um usuário"}.`);
      }
    }),
    bulkUpdate: workflowProjectProcedure(true).input(z.object({
      ...
      if (input.data.status === "Aprovado") await safeWorkflowNotification(input.projectId, "DCDs aprovados em lote", `${updated} DCD(s) foram aprovados por ${ctx.user.name || ctx.user.email || "um usuário"}.`);
      return { updated };
    }),
...
Retorne em formato markdown profissional. Não copie os fatos do exemplo e não invente transações ou apps ausentes do contexto.`;
  ...
  gaps: router({
    list: workflowProjectProcedure().input(z.object({ projectId: z.string(), ...paginationInput })).query(({ input }) =>
      wdb.listGaps(input.projectId, input)
    ),
    create: workflowProjectProcedure(true).input(z.object({
      projectId: z.string(),
    ...
    extractFromDcd: workflowProjectProcedure(true).input(z.object({
      ...
      if (!document || document.projectId !== input.projectId) throw new TRPCError({ code: "NOT_FOUND", message: "DCD não encontrado no projeto selecionado" });
...
[{"description": "...", "impact": "Alto|Médio|Baixo", "module": "SD|MM|FI|CO|..."}]`;
      ...
      const schema = z.array(z.object({ description: z.string().min(1), impact: z.enum(["Alto", "Médio", "Baixo"]), module: z.string().default("") })).max(500);
      ...
      for (let attempt = 0; attempt < 2 && !gapsList; attempt++) {
        ...
        const result = await invokeWorkflowLLM({ model: ai.model, messages: [{ role: "system", content: ai.systemPrompt }, { role: "user", content: prompt + correction }] });
        lastOutput = typeof result.choices?.[0]?.message?.content === "string" ? result.choices[0].message.content : "";
        try { const match = lastOutput.match(/\[[\s\S]*\]/); gapsList = match ? schema.parse(JSON.parse(match[0])) : null; } catch { gapsList = null; }
      }
      if (!gapsList) throw new TRPCError({ code: "BAD_GATEWAY", message: "A IA não retornou uma lista de gaps válida após nova tentativa" });
      const existing = await wdb.listGaps(input.projectId);
      const known = new Set(existing.map((gap: any) => legacyKey(gap.module, gap.description)));
      const created = [];
      ...
      await recordWorkflowAudit(ctx, input.projectId, "GAPS_EXTRACTED", "dcd", document.id, { extracted: created.length, ignored: gapsList.length - created.length });
  ...
  configurations: router({
    list: workflowProjectProcedure().input(z.object({ projectId: z.string(), ...paginationInput })).query(({ input }) =>
      wdb.listConfigurations(input.projectId, input)
    ),
    create: workflowProjectProcedure(true).input(z.object({
      projectId: z.string(),
      module: z.string().optional(),
    ...
    generateFromDcd: workflowProjectProcedure(true).input(z.object({
      projectId: z.string().min(1), dcdId: z.string().min(1),
    })).mutation(async ({ ctx, input }) => {
      const document = await wdb.getDcdDocument(input.dcdId);
      if (!document || document.projectId !== input.projectId) throw new TRPCError({ code: "NOT_FOUND", message: "DCD não encontrado no projeto selecionado" });
      const schema = z.array(z.object({
        description: z.string().min(1), module: z.string().default(""),
        category: z.enum(["Configuração", "Customizing", "Extensão", "Migração"]).default("Configuração"),
      })).max(500);
      ...
      for (let attempt = 0; attempt < 2 && !parsed; attempt++) {
        ...
        const result = await invokeWorkflowLLM({ model: ai.model, messages: [{ role: "system", content: ai.systemPrompt }, { role: "user", content: prompt + correction }] });
        lastOutput = typeof result.choices?.[0]?.message?.content === "string" ? result.choices[0].message.content : "";
        try { const match = lastOutput.match(/\[[\s\S]*\]/); parsed = match ? schema.parse(JSON.parse(match[0])) : null; } catch { parsed = null; }
      }
      if (!parsed) throw new TRPCError({ code: "BAD_GATEWAY", message: "A IA não retornou uma lista de configurações válida após nova tentativa" });
      const existing = await wdb.listConfigurations(input.projectId);
      const known = new Set(existing.map((item: any) => legacyKey(item.module, item.description)));
      let added = 0; let ignored = 0;
      for (const item of parsed) {
        const key = legacyKey(item.module, item.description);
        if (known.has(key)) { ignored++; continue; }
        await wdb.createConfiguration({ id: nanoid(), projectId: input.projectId, module: item.module, category: item.category, description: item.description, responsible: "", status: "Pendente", notes: `Extraída de ${document.title}` });
        known.add(key); added++;
      }
      await recordWorkflowAudit(ctx, input.projectId, "CONFIGURATIONS_EXTRACTED", "dcd", document.id, { added, ignored });
      return { added, ignored };
    }),
  ...
  tests: router({
    list: workflowProjectProcedure().input(z.object({ projectId: z.string(), type: testTypeSchema.optional(), ...paginationInput })).query(async ({ input }) => {
      const rows = await wdb.listWorkflowTestCases(input.projectId, input);
      return input.type ? rows.filter((item: any) => item.type === input.type) : rows;
    }),
    create: workflowProjectProcedure(true).input(z.object({
      projectId: z.string(), type: testTypeSchema, code: z.string().optional(), title: z.string().trim().min(1),
      description: z.string().optional(), module: z.string().optional(), requirementId: z.string().optional(), scopeItemId: z.string().optional(), dcdId: z.string().optional(),
      preconditions: z.string().optional(), steps: z.string().optional(), expectedResult: z.string().optional(), responsible: z.string().optional(), status: testStatusSchema.optional(),
    })).mutation(async ({ ctx, input }) => {
      for (const [table, id] of [["client_requirements", input.requirementId], ["scope_items", input.scopeItemId], ["dcd_documents", input.dcdId]] as const) {
        if (id && await wdb.getWorkflowEntityProjectId(table, id) !== input.projectId) throw new TRPCError({ code: "BAD_REQUEST", message: "Um vínculo do caso de teste não pertence ao projeto selecionado" });
      }
      const id = nanoid();
      const created = await wdb.createWorkflowTestCase({ id, projectId: input.projectId, type: input.type, code: input.code || "", title: input.title, description: input.description, module: input.module || "", requirementId: input.requirementId || "", scopeItemId: input.scopeItemId || "", dcdId: input.dcdId || "", preconditions: input.preconditions, steps: input.steps, expectedResult: input.expectedResult, responsible: input.responsible || "", status: input.status || "Não iniciado" });
      await recordWorkflowAudit(ctx, input.projectId, "TEST_CASE_CREATED", "test_case", id, { type: input.type, title: input.title });
      return created;
    }),
    update: workflowEntityProcedure("workflow_test_cases", true).input(z.object({ id: z.string(), data: z.object({
      type: testTypeSchema.optional(), code: z.string().optional(), title: z.string().min(1).optional(), description: z.string().optional(), module: z.string().optional(),
      preconditions: z.string().optional(), steps: z.string().optional(), expectedResult: z.string().optional(), actualResult: z.string().optional(), responsible: z.string().optional(), evidence: z.string().optional(), status: testStatusSchema.optional(), executedAt: z.string().optional(),
    }) })).mutation(async ({ ctx, input }) => {
      const projectId = await wdb.getWorkflowEntityProjectId("workflow_test_cases", input.id);
      await wdb.updateWorkflowTestCase(input.id, input.data);
      if (projectId) await recordWorkflowAudit(ctx, projectId, "TEST_CASE_UPDATED", "test_case", input.id, { fields: Object.keys(input.data), status: input.data.status });
    }),
    bulkUpdate: workflowProjectProcedure(true).input(z.object({ projectId: z.string(), ids: z.array(z.string()).min(1).max(500), data: z.object({ responsible: z.string().optional(), status: testStatusSchema.optional(), executedAt: z.string().optional() }) })).mutation(async ({ ctx, input }) => {
      await assertEntitiesBelongToProject("workflow_test_cases", input.ids, input.projectId);
      const updated = await wdb.bulkUpdateWorkflowTestCases(input.ids, input.data);
      await recordWorkflowAudit(ctx, input.projectId, "TEST_CASES_BULK_UPDATED", "test_cases", input.ids[0], { ids: input.ids, data: input.data, updated });
      return { updated };
    }),
    delete: workflowEntityProcedure("workflow_test_cases", true).input(z.object({ id: z.string() })).mutation(({ input }) => wdb.deleteWorkflowTestCase(input.id)),
  }),
